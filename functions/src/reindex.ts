...
const skipGenerateMissing =
  process.argv.includes('--skip-generate') || process.env.REINDEX_SKIP_GENERATE === '1';
...
const reindexNotes = async (): Promise<ReindexStats> => {
  ...
  while (true) {
    let query = notesRef.orderBy(admin.firestore.FieldPath.documentId()).limit(PAGE_SIZE);
  ...
  while (true) {
    ...
    for (const doc of snap.docs) {
      ...
      if (hasValidStored && !forceRegenerate) {
        ...
        if (embeddingArray.length === 0 && hasValidStored && forceRegenerate) {
          console.warn(
            `Force regenerate failed for section ${doc.ref.path}; keeping existing embedding.`,
          );
          embeddingArray = storedEmbedding;
        }
      } else if (hasValidStored) {
