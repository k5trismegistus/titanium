  ...
  async (event) => {
    ...
    try {
      ...
      const noteUpdate: admin.firestore.UpdateData<admin.firestore.DocumentData> = {
        ...
        salientKeywords: salientItems
          .filter((item) => item.type === 'keyword')
          .map((item) => item.text),
        salientClaims: salientItems
      ...
      await batch.commit();
      console.log(
        `Generated embeddings for note ${event.params.noteId}, ${sections.length} sections, and ${salientEmbeddingResults.length} salient items.`,
      );
    } catch (e) {
