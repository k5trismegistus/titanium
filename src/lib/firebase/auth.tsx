...
export function AuthProvider({ children }: { children: React.ReactNode }) {
    ...
    if (loading && !isDemoRoute) {
        return (
            <div className="h-[100dvh] w-screen flex items-center justify-center text-primary">
                Loading...
            </div>
        );
    }
                    ...
                    <div className="mt-8 grid gap-10 lg:grid-cols-[1.3fr_0.7fr] lg:items-start">
                        <div>
                            <div className="inline-flex items-center gap-2 rounded-full bg-white px-3 py-1 text-xs text-gray-500 shadow-sm border border-gray-100">
                                ...
                                {isJP
                                    ? '思考の流れを止めないノート'
                                    : 'Notes that keep thinking in motion'}
                            </div>
                                ...
                                <div className="mt-4 flex flex-col items-stretch gap-4 sm:flex-row sm:items-center">
                                    <ConceptStep
                                        ...
                                        description={
                                            isJP
                                                ? '素早く書いて、自動保存。'
                                                : 'Write fast with auto-save.'
                                        }
                                    />
                                    <span className="material-symbols-outlined mx-auto text-gray-300 rotate-90 sm:rotate-0">
                                        arrow_forward
                                    </span>
                                    <ConceptStep
                                        icon="link"
                                        ...
                                        description={
                                            isJP
                                                ? '文脈で関連ノートを発見。'
                                                : 'Discover related notes by context.'
                                        }
                                    />
                                    <span className="material-symbols-outlined mx-auto text-gray-300 rotate-90 sm:rotate-0">
                                        arrow_forward
                                    ...
                                    <ConceptStep
                                        ...
                                        description={
                                            isJP
                                                ? 'アイデアを合成して発展。'
                                                : 'Synthesize ideas into new insights.'
                                        }
                                    />
                                </div>
                            </div>
                        ...
                        <div className="bg-white p-8 rounded-2xl shadow-xl w-full text-center border border-gray-100">
                            ...
                            <p className="text-gray-500 mb-8">
                                {isJP
                                    ? 'Googleでログインして、あなたのノートを始めましょう。'
                                    : 'Sign in with Google to start your notes.'}
                            </p>
    ...
    return (
        <AuthContext.Provider value={{ user, loading, isAllowed }}>{children}</AuthContext.Provider>
    );
}
