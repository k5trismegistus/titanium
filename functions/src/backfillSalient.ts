...
const forceRegenerate = process.argv.includes("--force") || process.env.SALIENT_BACKFILL_FORCE === "1";
...
if (!projectId) {
    throw new Error("Missing project ID. Set GCLOUD_PROJECT/FIREBASE_PROJECT_ID or configure .firebaserc.");
}
...
const processNote = async (
    ...
    const validItems = embeddedItems.filter((item): item is NonNullable<typeof item> => item !== null);
    ...
    batch.set(doc.ref, {
        salientKeywords: validItems.filter((item) => item.type === "keyword").map((item) => item.text),
...
const run = async () => {
    ...
    console.log(`pageSize=${PAGE_SIZE} maxItems=${MAX_SALIENT_ITEMS} force=${forceRegenerate} dryRun=${dryRun} targetUid=${targetUid || "all"}`);
    ...
    while (true) {
        ...
        console.log(`Progress scanned=${stats.scanned} updated=${stats.updated} failed=${stats.failed}`);
    }
