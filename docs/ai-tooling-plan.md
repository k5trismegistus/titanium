...
調査基準: 2026-09-23 の作業ツリー。実装済みと今後の候補を分けて記す。
...
- Node 24 の標準テストランナーで選択範囲アンカーのテストを追加した。編集可能な手動確認画面を `/tests/editorHarness.html` に置いた。
...
| 3      | 重要な編集・検索処理のテスト            | Markdown の往復、2048 次元への射影、候補集約、Mix の保存境界を検証する                                |
